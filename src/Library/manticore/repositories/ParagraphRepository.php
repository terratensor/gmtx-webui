...
class ParagraphRepository
{
     ...
     */
    public function findByQueryStringNew(SearchForm $form): Search
    {
     ...
     */
    public function findByQueryStringMatch(SearchForm $form): Search
    {
     ...
     */
    public function findByMatchPhrase(SearchForm $form): Search
    {
        $this->prepareFilters($form);

        if ($form->query !== '') {
            $search = $this->search->phrase($form->query, 'content');
        } else {
            $search = $this->search->search($form->query);
        }
     ...
     */
    public function findByContext(SearchForm $form): Search
    {
    ...
    // Подготовка фильтров
    private function prepareFilters(SearchForm $form): void
    {
        if ($form->genre !== '') {
            if ($form->genre === SearchHelper::EMPTY_GENRE) {
                $this->search->filter('genre', 'in', "");
            } else {
